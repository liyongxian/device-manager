...
public class V2QueryService {
	...
	public NodeRespones queryByUri(String requestJson,String namespace,String id) throws Exception,CheckDefineException{
		NodeRespones response=new NodeRespones();
		String url=host+"/v2/model/nodes/"+namespace+"/access";        
     ...
     * 深度查询
     */
    public NodeRespones depth(String requestJson,String namespace,int maxSize)throws Exception,CheckDefineException{
    	NodeRespones response=new NodeRespones();
    	//采用深度查询，查询上级节点
    	String url=host+"/v2/model/query/depth/"+namespace+"?maxSize="+maxSize;
    	response=restTemplateService.post(url, requestJson);
    	if(response.getCode().equals(Constans.MODEL_RESPONSE_FAIL)){
			throw new CheckDefineException("model servie Exception when excut : "+url+"param:"+requestJson+response.getMsg());
    	}
     ...
     * 鏌ヨ鎸囧畾鐨勫叧绯�
     */
    public NodeRespones whetherHasRelation(String requestJson)throws Exception,CheckDefineException{
    	NodeRespones response=new NodeRespones();
    	//閲囩敤娣卞害鏌ヨ锛屾煡璇笂绾ц妭鐐�
    	String url=host+"/v2/model/query/whetherHasRelation";
    	response=restTemplateService.post(url, requestJson);
    	if(response.getCode().equals(Constans.MODEL_RESPONSE_FAIL)){
			throw new CheckDefineException("model servie Exception when excut : "+url+"param:"+requestJson+response.getMsg());
    	}
    ...
    public NodeRespones depth(String namespace,int maxSize,int depth,boolean forward,String id,List<String> fields,List<String> relations,List<DepthSpecifyParam> specifyList)throws Exception,CheckDefineException{
    	NodeRespones response=new NodeRespones();
    	//采用深度查询，查询上级节点
